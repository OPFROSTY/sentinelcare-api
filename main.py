...
def create_checkin(payload: CheckinCreate):
    ...
    return {"success": True, "risco": risco, "checkin": inserted.data[0] if inserted.data else None}


@app.get("/users/{user_id}/checkins")
def list_checkins(user_id: str):
    user = supabase.table("users").select("id").eq("id", user_id).limit(1).execute()
    if not user.data:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    ...
    return {"success": True, "checkins": response.data}
